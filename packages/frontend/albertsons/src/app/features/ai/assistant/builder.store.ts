...
import { BUILDER_ENABLED_VIEWS } from '@/features/ai/assistant/constants';
...
import { useBuilderMessages } from '@/features/ai/assistant/composables/useBuilderMessages';
import {
	chatWithBuilder,
	getAiSessions,
	getBuilderCredits,
...
import { generateMessageId, createBuilderPayload } from '@/features/ai/assistant/builder.utils';
import {
	useBuilderTodos,
...
import { AI_THINKING_STATUS } from '../../../../utils/constants';
...
export const useBuilderStore = defineStore('albertsons_' + STORES.BUILDER, () => {
	// Core state
	...
	function sendChatMessage_bak(options: {
		text: string;
		source?: 'chat' | 'canvas';
	...
	async function loadSessions() {
		...
		try {
			const response = { sessions: [] }; //await getAiSessions(rootStore.restApiContext, workflowId);
	...
	/**
	 * CUSTOM BUILDER CODE STARTS HERE
	 * Write all the custom logics and functions here
	 * Feel free to re-use anything that you want from in built functions without actually changing them
	 */

	type AssistantMessageStatus = keyof typeof AI_THINKING_STATUS;

	function sleep(ms: any) {
		return new Promise((resolve) => setTimeout(resolve, ms));
	}

	const assistantMessageIllusion = () => {
		addLoadingAssistantMessage(AI_THINKING_STATUS.THINKING);
		let index = 1;

		intervalId.value = setInterval(() => {
			const key: AssistantMessageStatus = Object.keys(AI_THINKING_STATUS)[index] as any;
			addLoadingAssistantMessage(AI_THINKING_STATUS[key]);
			index++;

			// Saving the last message to generate node data if node data exists.
			if (index === Object.keys(AI_THINKING_STATUS).length - 1) {
				clearInterval(intervalId.value); // auto-stop after 3 messages
			}
		}, 2000);
	};
	...
	const formatUserMessage = (content: string, id: string): ChatUI.AssistantMessage => {
		return {
			id,
			role: 'user',
			type: 'text',
	...
	/* Duplicate type of functions are intentional, to modify and add features */
	const formatAssistantMessage = (content: string, id: string): ChatUI.AssistantMessage => {
		return {
			id,
			role: 'assistant',
			type: 'text',
	...
	const onUserMessage = (text: string, userMessageId: string) => {
		const userMessage = formatUserMessage(text, userMessageId);
		chatMessages.value = [...chatMessages.value, userMessage];
		assistantMessageIllusion();
	};

	const onAssistantMessage = (text: string, assistantMessageId: string) => {
		const assistantMessage = formatAssistantMessage(text, assistantMessageId);
	...
	async function sendChatMessage(options: {
		...
		onUserMessage(text, userMessageId);
		...
		console.log('>>payload', payload?.workflowContext?.currentWorkflow);
		...
		try {
			// ---- REAL API CALL TO AI-BUILDER BACKEND ----
			streaming.value = true;
			assistantMessageIllusion();
			...
			const response = await fetch('http://localhost:8000/ai-builder', {
				method: 'POST',
				headers: {
			...
			const botText =
				apiResponse.botMessage ??
				'Something went wrong, but no botMessage was returned from the AI Builder.';
			onAssistantMessage(botText, assistantId);

			if (apiResponse.status === 'built') {
				// Re-fetch workflow from n8n so UI shows new nodes
				const updatedWorkflow = await workflowsStore.fetchWorkflow({
					id: workflowsStore.workflowId,
				} as any);

				workflowsStore.setNodes(updatedWorkflow.nodes ?? []);
				workflowsStore.setConnections(updatedWorkflow.connections ?? []);
			}

			stopStreaming();
		} catch (e: unknown) {
			handleServiceError(e as Error, userMessageId, retry);
		}
	}

	/**
	 * CUSTOM BUILDER CODE ENDS HERE
	 */
